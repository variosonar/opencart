...
class ModelJournal3Filter extends Model {
	...
	public function getManufacturers($filter_data) {
		...
		$sql = "
			...
			LEFT JOIN `" . DB_PREFIX . "manufacturer_description` m2d ON (m.manufacturer_id = md.manufacturer_id) 
	...
	private function addFilters($filter_data, $query = null) {
		...
		if (Arr::get($filter_data, 'special')) {
			if (version_compare(VERSION, '4.1.0.0', '>=')) {
				$sql .= " AND EXISTS (SELECT product_id FROM `" . DB_PREFIX . "product_discount` ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->journal3_opencart->customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())))";
			} else {
